...
public class LoginService
{
    ...
    public void add_publisher(String username, String password, String company, String address, String email, String phoneNumber,ModelMap model) {
        List<Map<String, Object>> response = conn.queryForList(
                "SELECT * FROM Access WHERE username = ?", new Object[]{ username}
        );
        ...
        if (response.size() == 1){
            ...
            try {
                ...
                conn.execute( "insert into Publishers (user_id, company_name, address) values \n" +
                        "\t( (select user_id from Access where username = \"" + username + "\" and Access_type = \"publisher\"),\"" + company + "\",\"" + address + "\");");
